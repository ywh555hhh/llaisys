set -euo pipefail
WORK=/data/src/vllm-mrv100-probe
...
PYFILE=$WORK/scripts/run_vllm_variant.py
run_variant() {
  name=$1; shift
...
open('/data/src/vllm-mrv100-probe/artifacts/07_variants_summary.json','w').write(json.dumps(summary, indent=2, ensure_ascii=False))
PY
