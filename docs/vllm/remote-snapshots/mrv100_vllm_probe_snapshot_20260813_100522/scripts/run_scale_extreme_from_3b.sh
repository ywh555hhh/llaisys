...
MODELS=(
  ...
  "Qwen/Qwen2.5-14B-Instruct|/data/models/Qwen2.5-14B-Instruct|1024|0.95|1|32|cudagraph"
)
for item in "${MODELS[@]}"; do
  IFS='|' read -r model local_dir max_len gpu_mem batch max_tokens mode <<< "$item"
  slug=$(basename "$local_dir")
  log="$WORK/logs/09_extreme_${slug}.txt"
  out="$WORK/artifacts/09_extreme_${slug}.json"
  echo "===== $model $(date -Is) =====" | tee -a "$log"
  ...
  if [ ! -f "$local_dir/config.json" ] || ! find "$local_dir" -maxdepth 1 -type f \( -name '*.safetensors' -o -name '*.bin' \) | grep -q .; then
    echo "Downloading/resuming $model to $local_dir" | tee -a "$log"
...
PY
    ...
    echo "Model files already present for $model" | tee -a "$log"
  fi
  du -sh "$local_dir" | tee -a "$log"
  find "$local_dir" -maxdepth 1 -type f -printf '%f %s bytes\n' | sort | tee -a "$log"
  extra=()
...
summary=[]
for p in sorted(list(root.glob('08_scale_*.json')) + list(root.glob('09_extreme_*.json'))):
    ...
    if obj.get('ok'):
        ...
        row['total_s']=obj.get('timing_s',{}).get('total')
    ...
    summary.append(row)
(root/'09_extreme_summary.json').write_text(json.dumps(summary, indent=2, ensure_ascii=False))
